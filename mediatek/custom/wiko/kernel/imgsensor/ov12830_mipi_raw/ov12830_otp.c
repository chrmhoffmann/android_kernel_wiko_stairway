...
kal_bool otp_update_wb(unsigned char golden_rg, unsigned char golden_bg)
and
kal_bool otp_update_lenc(void)
and
...
//extern kal_uint16 OV12830_write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
extern kal_uint16 ov12830_write_id;
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);
#define OV12830_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, ov12830_write_id) //ov12830.write_id OV12830MIPI_WRITE_ID
...
*******************************************************************************/	
kal_bool otp_apply_wb(unsigned short golden_rg, unsigned short golden_bg)
{
...
* Return      :  1, success; 0, fail
*******************************************************************************/	
kal_bool otp_update_wb(unsigned short golden_rg, unsigned short golden_bg) 
{
	printk("start wb update,ov12830_write_id = 0x%x\n",ov12830_write_id);
...
int otp_update_BLC(void) 
{
	...
	if(k!=0)
	{
		if((k>=0x15) && (k<=0x40))
		{
			...
			printk("BLC value from 0x3d0b: %x\n",k);
			return 2;
		}
	}
	...
	if((k>=0x10) && (k<=0x40))
	{
		...
		OV12830_write_cmos_sensor(0x4000, temp);
		mDELAY(20); 
		printk("BLC value from 0x3d0a:%x\n",k);
		return 1;	
	}
	else
	{
		...
		OV12830_write_cmos_sensor(0x4000, temp);
		mDELAY(20); 
		printk("BLC value is set to 0x20\n");
	}
